...
export default function LandingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Access verification
      const accessRes = await fetch('https://meziani.org/api/validate-access', {
        method: 'POST',
      ...
      // Lead capture
      const response = await fetch('https://meziani.org/api/lead', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email,
        ...
        {status === 'success' ? (
          <div className="funnel-card">
            ...
            <button 
              ...
              onClick={() => { setStatus('idle'); setEmail(''); setCompany(''); setIndustry('') }}
            >
              Submit Another
            </button>
          </div>
