...
# Edge TTS voices (No API key needed!)
VOICES = {
    "David": "en-US-ChristopherNeural", # Confident adult male
    "Sarah": "en-US-AriaNeural",       # Clear adult female
    "Chloe": "en-US-AnaNeural",        # Young, energetic female (US)
    "Director": "en-US-GuyNeural"      # The user/caller
}

SYSTEM_PROMPT = """You are producing a hyper-realistic, continuous live radio broadcast.
...
3. FORMAT: EXACTLY ONE turn per line. No markdown formatting.
Syntax -> Speaker: [OVERLAP] (Optional) text.

Example:
David: The servers have been running hot...
Sarah: [OVERLAP] Yeah, that memory spike is wild.
Chloe: I think it's the DB migrations!
"""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            # We can receive bytes (audio) or text
            message = await websocket.receive()
            
            if "bytes" in message:
                # 1. Cancel ongoing LLM generation
                ...
                try:
                    ...
                    # 4. Give the user an AI voice playback
                ...
                if text_msg == "START":
                    session_history = []
                    directive = "Start a fresh broadcast segment. Just start talking!"
                    current_task = asyncio.create_task(stream_ollama(directive, websocket, session_history))
                elif text_msg == "CONTINUE":
                    directive = "Continue the conversation naturally or change the subject slightly."
                    current_task = asyncio.create_task(stream_ollama(directive, websocket, session_history))
