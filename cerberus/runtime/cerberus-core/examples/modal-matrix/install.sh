...
echo ">>> Cross-compiling cerberus for x86_64-linux-musl..."
(cd "$REPO_ROOT" && zig build -Dtarget=x86_64-linux-musl -Doptimize=ReleaseSmall -Dchannels=matrix)

BINARY="$REPO_ROOT/zig-out/bin/cerberus"
if [ ! -f "$BINARY" ]; then
  echo "Build failed — binary not found at $BINARY"
  exit 1
fi

cp "$BINARY" "$SCRIPT_DIR/cerberus-linux-musl"
echo "  Binary: $SCRIPT_DIR/cerberus-linux-musl ($(du -h "$SCRIPT_DIR/cerberus-linux-musl" | cut -f1))"
