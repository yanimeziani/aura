"""Deploy cerberus Matrix gateway on Modal.
...
Logs:
  modal app logs cerberus-matrix
"""
...
image = (
    ...
    .add_local_file(
        f"{example_dir}/cerberus-linux-musl",
        "/opt/cerberus",
    )
    .add_local_file(
...
def start_tailscale():
    ...
    hostname = os.environ.get("TAILSCALE_HOSTNAME", "cerberus-modal")
    subprocess.run(
        [
            "tailscale", "up",
            "--authkey", authkey,
...
def gateway():
    ...
    # Write patched config for cerberus
    config_dir = Path("/cerberus-data/.cerberus")
    config_dir.mkdir(parents=True, exist_ok=True)
    config_path = config_dir / "config.json"
    config_path.write_text(json.dumps(config, indent=2))

    # Set up workspace
    workspace = Path("/cerberus-data/workspace")
    workspace.mkdir(parents=True, exist_ok=True)

    # Expose GITHUB_TOKEN to git via .netrc
    token = os.environ.get("GITHUB_TOKEN", "")
    if token:
        netrc_path = Path("/cerberus-data/.netrc")
    ...
    env = os.environ.copy()
    env["HOME"] = "/cerberus-data"
    env["CERBERUS_WORKSPACE"] = "/cerberus-data/workspace"

    subprocess.Popen(
        ["/opt/cerberus", "gateway", "--port", "3000", "--host", "::"],
        env=env,
    )
