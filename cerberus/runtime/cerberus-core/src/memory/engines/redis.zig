...
//! Designed for distributed memory sharing across multiple cerberus instances.
...
test "integration: redis count" {
    ...
    var mem = try RedisMemory.init(std.testing.allocator, .{
        .key_prefix = "cerberus_test_count",
    });
    defer mem.deinit();
...
test "integration: redis recall substring" {
    ...
    var mem = try RedisMemory.init(std.testing.allocator, .{
        .key_prefix = "cerberus_test_recall",
    });
    defer mem.deinit();
...
test "integration: redis forget" {
    ...
    var mem = try RedisMemory.init(std.testing.allocator, .{
        .key_prefix = "cerberus_test_forget",
    });
    defer mem.deinit();
...
test "integration: redis health check" {
    ...
    var mem = try RedisMemory.init(std.testing.allocator, .{
        .key_prefix = "cerberus_test_health",
    });
    defer mem.deinit();
