...
pub const Config = struct {
    ...
    pub fn load(backing_allocator: std.mem.Allocator) !Config {
        ...
        const config_dir = try std.fs.path.join(allocator, &.{ home, ".cerberus" });
    ...
    /// Apply CERBERUS_* environment variable overrides.
    pub fn applyEnvOverrides(self: *Config) void {
        // Provider
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_PROVIDER")) |prov| {
            self.default_provider = prov;
        } else |_| {}

        // Model
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_MODEL")) |model| {
            self.default_model = model;
        } else |_| {}

        // Temperature
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_TEMPERATURE")) |temp_str| {
        ...
        // Gateway port
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_GATEWAY_PORT")) |port_str| {
        ...
        // Gateway host
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_GATEWAY_HOST")) |host| {
            self.gateway.host = host;
        } else |_| {}

        // Workspace
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_WORKSPACE")) |ws| {
            self.workspace_dir = ws;
        } else |_| {}

        // Allow public bind
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_ALLOW_PUBLIC_BIND")) |val| {
    ...
    pub fn printValidationError(err: ValidationError) void {
        switch (err) {
            ...
            ValidationError.NoDefaultModel => std.debug.print(
                "No default model configured. Set agents.defaults.model.primary in ~/.cerberus/config.json or run `cerberus onboard`.\n",
                .{},
            ),
...
test "validation rejects malformed web origin entry" {
    const origins = [_][]const u8{"relay.cerberus.io"};
    const web_accounts = [_]WebConfig{
        .{
            .account_id = "default",
            .path = "/ws",
...
test "validation rejects malformed relay url and agent id" {
    const bad_url_accounts = [_]WebConfig{
        .{
            .account_id = "default",
            .transport = "relay",
            .relay_url = "https://relay.cerberus.io/ws",
            .relay_agent_id = "agent-1",
            .relay_token = "relay-token-0123456789",
        },
    };
    ...
    const bad_agent_accounts = [_]WebConfig{
        .{
            .account_id = "default",
            .transport = "relay",
            .relay_url = "wss://relay.cerberus.io/ws",
...
test "applyEnvOverrides does not crash on default config" {
    ...
    // Should not crash even when no CERBERUS_* env vars are set
    cfg.applyEnvOverrides();
...
test "save writes provider native_tools when false" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, content, "\"user_agent\": \"cerberus-test/1.0\"") != null);
}
...
test "save escapes provider string fields" {
    ...
    try std.testing.expectEqualStrings("cerberus \"agent\"", openai.get("user_agent").?.string);
}
...
test "parse web accounts with auth token path and allowed origins" {
    const allocator = std.testing.allocator;
    const json =
        \\{"channels": {"web": {"accounts": {"default": {"listen": "0.0.0.0", "port": 32123, "path": "/ws", "auth_token": "relay-token-123456", "allowed_origins": ["https://relay.cerberus.io", "chrome-extension://abc"]}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
    ...
    try std.testing.expectEqualStrings("https://relay.cerberus.io", wc.allowed_origins[0]);
...
test "parse web relay account fields" {
    const allocator = std.testing.allocator;
    const json =
        \\{"channels": {"web": {"accounts": {"default": {"transport": "relay", "relay_url": "wss://relay.cerberus.io/ws/agent", "relay_agent_id": "edge-1", "relay_token": "relay-token-999999", "relay_token_ttl_secs": 7200, "relay_pairing_code_ttl_secs": 180, "relay_ui_token_ttl_secs": 86400, "relay_e2e_required": true}}}}}
    ;
    var cfg = Config{ .workspace_dir = "/tmp/yc", .config_path = "/tmp/yc/config.json", .allocator = allocator };
    try cfg.parseJson(json);
    ...
    try std.testing.expectEqualStrings("wss://relay.cerberus.io/ws/agent", wc.relay_url.?);
...
test "save includes nostr channel when configured" {
    ...
    const tmp_path = "/tmp/cerberus_test_nostr_save.json";
...
test "save includes dm_relays in nostr section" {
    const allocator = std.testing.allocator;
    const tmp_path = "/tmp/cerberus_test_dm_relays_save.json";
...
test "dm_relays round-trips through save and load" {
    const allocator = std.testing.allocator;
    const tmp_path = "/tmp/cerberus_test_dm_relays_roundtrip.json";
...
test "nostr display_name with special chars round-trips correctly" {
    const allocator = std.testing.allocator;
    const tmp_path = "/tmp/cerberus_test_nostr_escape.json";
    ...
    try std.testing.expect(std.mem.indexOf(u8, raw, "\\\"Cerberus\\\"") != null);
    ...
    const ns = loaded.channels.nostr.?;
    try std.testing.expectEqualStrings("Bot \"Cerberus\" v1", ns.display_name);
