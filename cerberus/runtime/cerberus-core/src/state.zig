...
//! Persisted to `~/.cerberus/state.json` with atomic writes (temp + rename).
...
test "defaultStatePath" {
    const path = try defaultStatePath(testing.allocator, "/home/user/.cerberus");
    defer testing.allocator.free(path);
    try testing.expectEqualStrings("/home/user/.cerberus/state.json", path);
}
