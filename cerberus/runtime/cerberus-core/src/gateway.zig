//! HTTP Gateway — lightweight HTTP server for cerberus.
//!
...
/// Process an incoming message by spawning `cerberus agent -m "..."`.
...
pub fn processIncomingMessage(allocator: std.mem.Allocator, message: []const u8) ![]u8 {
    ...
    const self_path = std.fs.selfExePath(&self_buf) catch "cerberus";
