...
//! for `cerberus agent`) and the streaming stdout callback.
...
/// This is the main entry point called by `cerberus agent`.
pub fn run(allocator: std.mem.Allocator, args: []const [:0]const u8) !void {
    var cfg = Config.load(allocator) catch {
        log.err("No config found. Run `cerberus onboard` first.", .{});
        return;
    };
    defer cfg.deinit();
    ...
    // Single message mode: cerberus agent -m "hello"
    ...
    try w.print("cerberus Agent -- Interactive Mode\n", .{});
