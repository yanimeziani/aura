//! Doctor -- system diagnostics for cerberus.
//!
...
//!   - Sandbox, cron status, channel connectivity (cerberus-specific)
...
pub fn runDoctor(
    ...
    if (err_count > 0) {
        try writer.writeAll("Run 'cerberus doctor --fix' or check your config.\n");
    }
}
...
pub fn run(allocator: std.mem.Allocator) !void {
    ...
    var cfg = Config.load(allocator) catch {
        const prefix = if (color)
            ...
            "[ERR]";
        try stdout.print("{s} No config found -- run `cerberus onboard` first\n", .{prefix});
        try stdout.flush();
        return;
    };
    defer cfg.deinit();
...
pub fn checkConfigSemantics(
    ...
    if (has_channel) {
        ...
        try items.append(allocator, DiagItem.warn(cat, "no channels configured -- run `cerberus onboard` to set one up"));
    }
}
...
pub fn checkWorkspace(
    ...
    // Writable probe
    const probe_name = ".cerberus_doctor_probe";
...
test "checkDaemonState handles missing file" {
    ...
    var cfg = testConfig();
    cfg.config_path = "/tmp/nonexistent-cerberus-test/config.json";
...
fn testConfig() Config {
    return Config{
        .workspace_dir = "/tmp/cerberus-test-workspace",
