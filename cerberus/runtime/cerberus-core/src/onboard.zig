//! Onboarding — interactive setup wizard and quick setup for cerberus.
//!
...
const WORKSPACE_STATE_DIR = ".cerberus";
...
///
/// Uses file-based cache at `~/.cerberus/state/models_cache.json` with 12h TTL.
...
pub fn fetchModels(allocator: std.mem.Allocator, provider: []const u8, api_key: ?[]const u8) ![][]const u8 {
    ...
    const state_dir = try std.fs.path.join(allocator, &.{ home, ".cerberus", "state" });
...
pub fn runQuickSetup(allocator: std.mem.Allocator, api_key: ?[]const u8, provider: ?[]const u8, memory_backend: ?[]const u8) !void {
    ...
    if (cfg.defaultProviderKey() == null) {
        ...
        try stdout.writeAll("    2. Chat:              cerberus agent -m \"Hello!\"\n");
        try stdout.writeAll("    3. Gateway:           cerberus gateway\n");
    } else {
        try stdout.writeAll("    1. Chat:     cerberus agent -m \"Hello!\"\n");
        try stdout.writeAll("    2. Gateway:  cerberus gateway\n");
        try stdout.writeAll("    3. Status:   cerberus status\n");
    }
    try stdout.writeAll("\n");
    try stdout.flush();
}
...
pub fn runChannelsOnly(allocator: std.mem.Allocator) !void {
    ...
    var cfg = Config.load(allocator) catch {
        try stdout.writeAll("No existing config found. Run `cerberus onboard` first.\n");
        try stdout.flush();
...
pub fn runWizard(allocator: std.mem.Allocator) !void {
    ...
    try out.writeAll("  Welcome to cerberus -- the fastest, smallest AI assistant.\n");
    ...
    if (cfg.defaultProviderKey() == null) {
        ...
        try out.writeAll("    2. Chat:              cerberus agent -m \"Hello!\"\n");
        try out.writeAll("    3. Gateway:           cerberus gateway\n");
    } else {
        try out.writeAll("    1. Chat:     cerberus agent -m \"Hello!\"\n");
        try out.writeAll("    2. Gateway:  cerberus gateway\n");
        try out.writeAll("    3. Status:   cerberus status\n");
    }
    try out.writeAll("\n");
    try out.flush();
}
...
/// Saves results to ~/.cerberus/models_cache.json.
pub fn runModelsRefresh(allocator: std.mem.Allocator) !void {
    ...
    defer allocator.free(home);
    const cache_path = try std.fs.path.join(allocator, &.{ home, ".cerberus", "models_cache.json" });
    defer allocator.free(cache_path);
    const cache_dir = try std.fs.path.join(allocator, &.{ home, ".cerberus" });
...
fn getDefaultWorkspace(allocator: std.mem.Allocator) ![]const u8 {
    ...
    defer allocator.free(home);
    return std.fs.path.join(allocator, &.{ home, ".cerberus", "workspace" });
}

fn getDefaultConfigPath(allocator: std.mem.Allocator) ![]const u8 {
    const home = try platform.getHomeDir(allocator);
    defer allocator.free(home);
    return std.fs.path.join(allocator, &.{ home, ".cerberus", "config.json" });
}
...
test "ProjectContext default values" {
    ...
    try std.testing.expectEqualStrings("cerberus", ctx.agent_name);
...
test "BANNER is non-empty and contains cerberus branding" {
