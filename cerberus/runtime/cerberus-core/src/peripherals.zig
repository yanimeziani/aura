...
test "gpioRead returns result with state" {
    ...
    const p = rpi.peripheral();
    if (comptime builtin.os.tag == .linux) {
        // Only run on real RPi hardware — requires CERBERUS_GPIO_TEST=1
        if (platform.getEnvOrNull(std.testing.allocator, "CERBERUS_GPIO_TEST")) |v| std.testing.allocator.free(v) else return error.SkipZigTest;
