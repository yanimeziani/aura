...
from aura_runtime import aura_root, vault_file

# Repo root: AURA_ROOT env or directory containing this script
def _root() -> Path:
    return aura_root(Path(__file__).resolve().parent)
...
def _run_bash(script: Path, *args, env_extra: dict | None = None) -> int:
    """Run a bash script. On Windows try bash (Git Bash), else exit with message."""
    env = {**os.environ, "AURA_ROOT": str(_root()), "REPO_ROOT": str(_root()), **(env_extra or {})}
...
def cmd_help() -> None:
    ...
    print("AURA_ROOT defaults to repo root (this directory).")


def cmd_gateway(root: Path) -> int:
    port = os.environ.get("NEXA_GATEWAY_PORT", os.environ.get("AURA_GATEWAY_PORT", "9080"))
...
def cmd_vault(root: Path) -> int:
    return _run(
        ...
        env={**os.environ, "AURA_ROOT": str(root)},
    )


def cmd_docs_bundle(root: Path) -> int:
    env = {**os.environ, "AURA_ROOT": str(root), "REPO_ROOT": str(root)}
    return _run([_python(), str(root / "ops" / "scripts" / "build-aura-docs-bundle.py")], env=env)
...
def cmd_backup(root: Path) -> int:
    ...
    if not script.exists():
        ...
        return 1
    return _run_bash(script, *sys.argv[2:], env_extra={"AURA_ROOT": str(root)})
...
def cmd_publish_notebooklm(root: Path) -> int:
    ...
    if not script.exists():
        ...
        return 1
    return _run_bash(script, env_extra={"AURA_ROOT": str(root), "REPO_ROOT": str(root)})
...
def main() -> int:
    ...
    # Delegate to bash aura if present (for full command set on Unix, or Windows with Git Bash)
    bash_aura = root / "ops" / "bin" / "aura"
    bash = "bash" if platform.system() != "Windows" else _find_bash_windows()
    if bash_aura.exists() and bash:
        return subprocess.run([bash, str(bash_aura)] + sys.argv[1:], env={**os.environ, "AURA_ROOT": str(root)}, cwd=str(root)).returncode
