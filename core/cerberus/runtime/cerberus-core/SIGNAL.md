...
Run cerberus as a Signal chatbot using Docker Compose and a local `signal-cli` container.

## Architecture

```
Signal app  <-->  signal-cli container (:8080)  <-->  cerberus gateway (:3000)
```
...
docker compose -f docker-compose.yml -f docker-compose.signal.yml exec signal-cli signal-cli link -n cerberus
```
...
### 3. Build cerberus image

```bash
DOCKER_BUILDKIT=0 docker build -t cerberus:latest .
```
