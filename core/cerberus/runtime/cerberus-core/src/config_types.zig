...
pub const WebConfig = struct {
    ...
    account_id: []const u8 = "default",
    /// "local" starts an inbound WS listener in cerberus.
    ...
    /// If null, WebChannel falls back to env (CERBERUS_WEB_TOKEN/CERBERUS_GATEWAY_TOKEN/OPENCLAW_GATEWAY_TOKEN),
    ...
    /// If omitted, relay lifecycle resolves token from CERBERUS_RELAY_TOKEN,
...
pub const NostrConfig = struct {
    ...
    display_name: []const u8 = "Cerberus",
...
pub const MemoryVectorStoreConfig = struct {
    ...
    qdrant_collection: []const u8 = "cerberus_memories",
...
test "WebConfig origin validation accepts wildcard and absolute origins" {
    ...
    try std.testing.expect(WebConfig.isValidAllowedOrigin("https://relay.cerberus.io"));
    try std.testing.expect(WebConfig.isValidAllowedOrigin("https://relay.cerberus.io/"));
    try std.testing.expect(WebConfig.isValidAllowedOrigin("chrome-extension://abcdefghijklmnop"));
    try std.testing.expect(!WebConfig.isValidAllowedOrigin(""));
    try std.testing.expect(!WebConfig.isValidAllowedOrigin("relay.cerberus.io"));
    try std.testing.expect(!WebConfig.isValidAllowedOrigin("https://relay.cerberus.io/path"));
}
...
test "WebConfig relay URL validation requires wss authority" {
    try std.testing.expect(WebConfig.isValidRelayUrl("wss://relay.cerberus.io/ws"));
    try std.testing.expect(WebConfig.isValidRelayUrl("wss://relay.cerberus.io"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("ws://relay.cerberus.io/ws"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("https://relay.cerberus.io/ws"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("wss://"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("wss://relay.cerberus.io?x=1"));
    try std.testing.expect(!WebConfig.isValidRelayUrl("wss://relay.cerberus.io/ws#frag"));
}
