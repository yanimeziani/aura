//! Self-update command for cerberus.
//!
...
pub fn run(allocator: std.mem.Allocator, opts: Options) !void {
    ...
    std.debug.print("Restart cerberus to use the new version.\n", .{});
}
...
pub fn detectInstallMethod() !InstallMethod {
    ...
    // Check for docker
    if (std.mem.eql(u8, exe_path, "/cerberus")) {
        return .docker;
    }
...
fn printPackageManagerUpdate(method: InstallMethod) !void {
    ...
    const cmd = switch (method) {
        .nix => "nix-channel --update && nix-env -iA nixpkgs.cerberus",
        .homebrew => "brew upgrade cerberus",
        .docker => "docker pull ghcr.io/cerberus/cerberus:latest",
        else => unreachable,
    };
...
pub fn getLatestRelease(allocator: std.mem.Allocator) !ReleaseInfo {
    const url = "https://api.github.com/repos/cerberus/cerberus/releases/latest";
...
pub const PlatformTarget = enum {
    ...
    pub fn assetName(self: PlatformTarget) []const u8 {
        return switch (self) {
            .linux_x86_64 => "cerberus-linux-x86_64.bin",
            .linux_aarch64 => "cerberus-linux-aarch64.bin",
            .macos_aarch64 => "cerberus-macos-aarch64.bin",
            .macos_x86_64 => "cerberus-macos-x86_64.bin",
            .windows_x86_64 => "cerberus-windows-x86_64.exe",
        };
    }
};
...
fn findAssetUrl(allocator: std.mem.Allocator, asset_name: []const u8) ?[]const u8 {
    ...
    const base_url = "https://github.com/cerberus/cerberus/releases/latest/download/";
...
test "PlatformTarget.assetName covers all release assets" {
    try std.testing.expectEqualStrings("cerberus-linux-x86_64.bin", PlatformTarget.linux_x86_64.assetName());
    try std.testing.expectEqualStrings("cerberus-linux-aarch64.bin", PlatformTarget.linux_aarch64.assetName());
    try std.testing.expectEqualStrings("cerberus-macos-aarch64.bin", PlatformTarget.macos_aarch64.assetName());
    try std.testing.expectEqualStrings("cerberus-macos-x86_64.bin", PlatformTarget.macos_x86_64.assetName());
    try std.testing.expectEqualStrings("cerberus-windows-x86_64.exe", PlatformTarget.windows_x86_64.assetName());
}
