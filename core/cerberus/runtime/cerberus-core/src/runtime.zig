...
test "NativeRuntime storage path contains cerberus" {
    var native = NativeRuntime{};
    const rt = native.adapter();
    try std.testing.expect(std.mem.indexOf(u8, rt.storagePath(), "cerberus") != null);
}
...
test "DockerRuntime filesystem with mount" {
    ...
    const rt = docker.adapter();
    try std.testing.expect(rt.hasFilesystemAccess());
    try std.testing.expectEqualStrings("/workspace/.cerberus", rt.storagePath());
}
...
test "DockerRuntime filesystem without mount" {
    ...
    const rt = docker.adapter();
    try std.testing.expect(!rt.hasFilesystemAccess());
    try std.testing.expectEqualStrings("/tmp/.cerberus", rt.storagePath());
}
...
test "NativeRuntime storage path" {
    var native = NativeRuntime{};
    const rt = native.adapter();
    try std.testing.expectEqualStrings(".cerberus", rt.storagePath());
}
