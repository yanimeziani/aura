...
fn runGateway(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    var cfg = yc.config.Config.load(allocator) catch {
        std.debug.print("No config found -- run `cerberus onboard` first\n", .{});
        std.process.exit(1);
    };
    defer cfg.deinit();
...
fn runService(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print("Usage: cerberus service <install|start|stop|restart|status|uninstall>\n", .{});
        std.process.exit(1);
    }
    ...
    yc.service.handleCommand(allocator, service_cmd, cfg.config_path) catch |err| {
        ...
        switch (any_err) {
            ...
            error.SystemctlUnavailable => {
                ...
                std.debug.print("Run `cerberus gateway` in the foreground or use another supervisor.\n", .{});
            },
            error.SystemdUserUnavailable => {
                std.debug.print("systemd user services are unavailable (`systemctl --user`).\n", .{});
                std.debug.print("Verify with `systemctl --user status` or run `cerberus gateway` in the foreground.\n", .{});
            },
            error.CommandFailed => {
...
fn runCron(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print(
            \\Usage: cerberus cron <command> [args]
            \\
            \\Commands:
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        if (sub_args.len < 3) {
            std.debug.print("Usage: cerberus cron add <expression> <command>\n", .{});
            std.process.exit(1);
        }
        try yc.cron.cliAddJob(allocator, sub_args[1], sub_args[2]);
    } else if (std.mem.eql(u8, subcmd, "add-agent")) {
        if (sub_args.len < 3) {
            std.debug.print("Usage: cerberus cron add-agent <expression> <prompt> [--model <model>]\n", .{});
            std.process.exit(1);
        }
        ...
        if (sub_args.len < 3) {
            std.debug.print("Usage: cerberus cron once <delay> <command>\n", .{});
            std.process.exit(1);
        }
        try yc.cron.cliAddOnce(allocator, sub_args[1], sub_args[2]);
    } else if (std.mem.eql(u8, subcmd, "once-agent")) {
        if (sub_args.len < 3) {
            std.debug.print("Usage: cerberus cron once-agent <delay> <prompt> [--model <model>]\n", .{});
            std.process.exit(1);
        }
        var model: ?[]const u8 = null;
        var i: usize = 3;
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus cron runs <id>\n", .{});
            std.process.exit(1);
        }
...
fn runChannel(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print(
            \\Usage: cerberus channel <command> [args]
            \\
            \\Commands:
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus channel add <type>\n", .{});
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus channel remove <name>\n", .{});
            std.process.exit(1);
        }
...
fn runSkills(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print(
            \\Usage: cerberus skills <command> [args]
            \\
            \\Commands:
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus skills install <source>\n", .{});
            std.process.exit(1);
        }
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus skills remove <name>\n", .{});
            std.process.exit(1);
        }
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus skills info <name>\n", .{});
            std.process.exit(1);
        }
...
fn runHardware(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print(
            \\Usage: cerberus hardware <command> [args]
            \\
            \\Commands:
    ...
    if (std.mem.eql(u8, subcmd, "scan")) {
        ...
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus hardware flash <firmware_file> [--target <board>]\n", .{});
            std.process.exit(1);
        }
        std.debug.print("Flash not yet implemented. Firmware file: {s}\n", .{sub_args[1]});
    } else if (std.mem.eql(u8, subcmd, "monitor")) {
        std.debug.print("Monitor not yet implemented. Use `cerberus hardware scan` to discover devices first.\n", .{});
    } else {
...
fn runMigrate(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    if (sub_args.len < 1) {
        std.debug.print(
            \\Usage: cerberus migrate <source> [options]
            \\
            \\Sources:
...
fn printMemoryUsage() void {
    std.debug.print(
        \\Usage: cerberus memory <command> [args]
        \\
        \\Commands:
...
fn printWorkspaceUsage() void {
    std.debug.print(
        \\Usage: cerberus workspace <command> [args]
        \\
        \\Commands:
...
fn runMemory(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "forget")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus memory forget <key>\n", .{});
            std.process.exit(1);
        }
    ...
    if (std.mem.eql(u8, subcmd, "get")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus memory get <key>\n", .{});
            std.process.exit(1);
        }
        const key = sub_args[1];
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        while (i < sub_args.len) : (i += 1) {
            if (std.mem.eql(u8, sub_args[i], "--limit")) {
                if (i + 1 >= sub_args.len) {
                    std.debug.print("Usage: cerberus memory list [--category C] [--limit N]\n", .{});
                    std.process.exit(1);
                }
                i += 1;
    ...
    if (std.mem.eql(u8, subcmd, "search")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus memory search <query> [--limit N]\n", .{});
            std.process.exit(1);
        }
...
fn runCapabilities(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (sub_args.len > 0) {
        if (sub_args.len == 1 and (std.mem.eql(u8, sub_args[0], "--json") or std.mem.eql(u8, sub_args[0], "json"))) {
            ...
            std.debug.print("Usage: cerberus capabilities [--json]\n", .{});
            std.process.exit(1);
        }
    }
...
fn runModels(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "list")) {
        ...
        if (cfg_opt) |c| {
            ...
            std.debug.print("  (no config -- run `cerberus onboard` first)\n\n", .{});
        }
        ...
        std.debug.print("\nUse `cerberus models info <model>` for details.\n", .{});
    } else if (std.mem.eql(u8, subcmd, "info")) {
        if (sub_args.len < 2) {
            std.debug.print("Usage: cerberus models info <model>\n", .{});
            std.process.exit(1);
        }
        std.debug.print("Model: {s}\n", .{sub_args[1]});
        ...
        std.debug.print("Configure a provider first (cerberus onboard).\n", .{});
...
fn printOnboardUsage() void {
    std.debug.print(
        \\Usage: cerberus onboard [--interactive | --channels-only | [--api-key KEY] [--provider PROV] [--memory MEM]]
        \\
        \\Modes:
        \\  (default)         quick setup
        ...
        \\
        \\Examples:
        \\  cerberus onboard --api-key sk-... --provider openrouter
        \\  cerberus onboard --interactive
        \\
    , .{});
}
...
fn runChannelStart(allocator: std.mem.Allocator, args: []const []const u8) !void {
    if (args.len > 0 and std.mem.eql(u8, args[0], "--all")) {
        std.debug.print("Use `cerberus gateway` to start all configured channels/accounts.\n", .{});
        std.process.exit(1);
    }
...
fn runSignalChannel(allocator: std.mem.Allocator, args: []const []const u8, config: *const yc.config.Config, signal_config: yc.config.SignalConfig) !void {
    ...
    if (resolved_api_key == null and provider_kind != .openai_codex_provider and !has_fallback_credentials) {
        std.debug.print("No API key configured. Set env var or add to ~/.cerberus/config.json:\n", .{});
    ...
    std.debug.print("cerberus Signal bot starting...\n", .{});
    ...
    const security = @import("cerberus").security.policy;
...
fn runMatrixChannel(
    ...
    std.debug.print("cerberus Matrix bot starting...\n", .{});
    std.debug.print("  Provider: {s}\n", .{config.default_provider});
...
fn runTelegramChannel(allocator: std.mem.Allocator, args: []const []const u8, config: yc.config.Config, telegram_config: yc.config.TelegramConfig) !void {
    ...
    std.debug.print("cerberus telegram bot starting...\n", .{});
    std.debug.print("  Provider: {s}\n", .{config.default_provider});
    std.debug.print("  Model: {s}\n", .{model});
    std.debug.print("  Temperature: {d:.1}\n", .{temperature});
    if (allowed.len == 0) {
...
fn runAuth(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    if (std.mem.eql(u8, subcmd, "set-key")) {
        runAuthSetProviderKey(allocator, provider_name, rest);
        return;
    }

    if (std.mem.eql(u8, subcmd, "clear-key")) {
        runAuthClearProviderKey(allocator, provider_name);
        return;
    }

    if (std.mem.eql(u8, subcmd, "key-status")) {
        runAuthProviderKeyStatus(allocator, provider_name);
        return;
    }
    ...
    if (std.mem.eql(u8, subcmd, "login")) {
        ...
        if (auth_mod.loadCredential(allocator, codex.CREDENTIAL_KEY) catch null) |token| {
            ...
            std.debug.print("  Run `cerberus auth login openai-codex` to authenticate.\n", .{});
        }
...
fn runUpdate(allocator: std.mem.Allocator, sub_args: []const []const u8) !void {
    ...
    while (i < sub_args.len) : (i += 1) {
        if (std.mem.eql(u8, sub_args[i], "--check")) {
            ...
            std.debug.print("Usage: cerberus update [--check] [--yes]\n", .{});
            std.process.exit(1);
        }
    }
...
fn printAuthUsage() void {
    std.debug.print(
        \\Usage: cerberus auth <command> <provider> [options]
        \\
        \\Commands:
        ...
        \\  clear-key <provider>                Remove provider API key from config
        \\  key-status <provider>               Show provider key status from config/env
        \\
        \\Providers:
        \\  openai-codex    ChatGPT Plus/Pro subscription (OAuth)
        \\  <direct-model-provider>             e.g. openai, openrouter, anthropic, groq
        \\
        \\Examples:
        ...
        \\  cerberus auth logout openai-codex
        \\
    , .{});
}

fn runAuthSetProviderKey(allocator: std.mem.Allocator, provider_name_raw: []const u8, rest: []const []const u8) void {
    const provider_name = yc.onboard.canonicalProviderName(provider_name_raw);
    if (std.mem.eql(u8, provider_name, "openai-codex")) {
        std.debug.print("openai-codex uses OAuth credentials, not API keys.\n", .{});
        std.debug.print("Run: cerberus auth login openai-codex\n", .{});
        std.process.exit(1);
    }
    ...
    for (rest) |arg| {
        if (std.mem.eql(u8, arg, "--stdin")) {
            ...
            std.debug.print("Unknown option for set-key: {s}\n", .{arg});
            std.process.exit(1);
        }
    }
    ...
    const token = if (from_stdin)
        readTokenFromStdin(allocator)
    else
        readTokenInteractive(allocator, provider_name);
    defer allocator.free(token);

    if (token.len == 0) {
        std.debug.print("Empty token. Nothing was saved.\n", .{});
        std.process.exit(1);
    }

    const config_dir = std.fs.path.dirname(cfg.config_path) orelse {
        std.debug.print("Invalid config path: {s}\n", .{cfg.config_path});
        std.process.exit(1);
    };
    const store = yc.security.SecretStore.init(config_dir, cfg.secrets.encrypt);
    const encrypted_token = store.encryptSecret(allocator, token) catch {
        std.debug.print("Failed to encrypt token for {s}.\n", .{provider_name});
        std.process.exit(1);
    };
    defer allocator.free(encrypted_token);

    upsertProviderApiKey(&cfg, provider_name, encrypted_token) catch {
        std.debug.print("Failed to update provider entry for {s}.\n", .{provider_name});
        std.process.exit(1);
    };

    cfg.save() catch {
        std.debug.print("Failed to save config file: {s}\n", .{cfg.config_path});
        std.process.exit(1);
    };

    const env_var = yc.onboard.providerEnvVar(provider_name);
    std.debug.print("Stored API key for {s} in config (encrypted at rest).\n", .{provider_name});
    std.debug.print("Tip: You can still override at runtime with {s}.\n", .{env_var});
}

fn runAuthClearProviderKey(allocator: std.mem.Allocator, provider_name_raw: []const u8) void {
    const provider_name = yc.onboard.canonicalProviderName(provider_name_raw);
    var cfg = yc.config.Config.load(allocator) catch {
        std.debug.print("No config found -- run `cerberus onboard` first\n", .{});
        std.process.exit(1);
    ...
    const cleared = clearProviderApiKey(&cfg, provider_name) catch {
        std.debug.print("Failed to update provider entry for {s}.\n", .{provider_name});
        std.process.exit(1);
    };

    if (!cleared) {
        std.debug.print("No stored config key found for {s}.\n", .{provider_name});
        return;
    }
    ...
    std.debug.print("Removed stored config key for {s}.\n", .{provider_name});
}

fn runAuthProviderKeyStatus(allocator: std.mem.Allocator, provider_name_raw: []const u8) void {
    const provider_name = yc.onboard.canonicalProviderName(provider_name_raw);
    var cfg = yc.config.Config.load(allocator) catch {
        std.debug.print("No config found -- run `cerberus onboard` first\n", .{});
        std.process.exit(1);
    ...
    const key = cfg.getProviderKey(provider_name);
    if (key) |k| {
        const trimmed = std.mem.trim(u8, k, " \t\r\n");
        if (trimmed.len > 0) {
            const enc = yc.security.SecretStore.isEncrypted(trimmed);
            std.debug.print("{s}: config key present ({s}).\n", .{ provider_name, if (enc) "enc2" else "plaintext" });
        } else {
            std.debug.print("{s}: config key entry is empty.\n", .{provider_name});
        }
    } else {
        std.debug.print("{s}: no config key set.\n", .{provider_name});
    }

    const env_var = yc.onboard.providerEnvVar(provider_name);
    const env_value = std.process.getEnvVarOwned(allocator, env_var) catch null;
    defer if (env_value) |v| allocator.free(v);
    if (env_value) |v| {
        const trimmed = std.mem.trim(u8, v, " \t\r\n");
        if (trimmed.len > 0) {
            std.debug.print("{s}: env override present ({s}).\n", .{ provider_name, env_var });
        } else {
            std.debug.print("{s}: env override {s} is empty.\n", .{ provider_name, env_var });
        }
    } else {
        std.debug.print("{s}: env override not set ({s}).\n", .{ provider_name, env_var });
    }
}

fn upsertProviderApiKey(cfg: *yc.config.Config, provider_name: []const u8, api_key: []const u8) !void {
    ...
    const needs_new = index == null;
    const new_len = if (needs_new) cfg.providers.len + 1 else cfg.providers.len;
    const providers_new = try cfg.allocator.alloc(yc.config.ProviderEntry, new_len);

    for (cfg.providers, 0..) |entry, i| {
        providers_new[i] = entry;
    }

    if (index) |idx| {
        var updated = providers_new[idx];
        updated.api_key = try cfg.allocator.dupe(u8, api_key);
        providers_new[idx] = updated;
    } else {
        providers_new[new_len - 1] = .{
            .name = try cfg.allocator.dupe(u8, provider_name),
            .api_key = try cfg.allocator.dupe(u8, api_key),
        };
    }

    cfg.providers = providers_new;
}

fn clearProviderApiKey(cfg: *yc.config.Config, provider_name: []const u8) !bool {
    for (cfg.providers, 0..) |entry, i| {
        if (!std.mem.eql(u8, entry.name, provider_name)) continue;
        var providers_new = try cfg.allocator.alloc(yc.config.ProviderEntry, cfg.providers.len);
        for (cfg.providers, 0..) |src, j| {
            providers_new[j] = src;
        }

        var updated = providers_new[i];
        updated.api_key = null;
        providers_new[i] = updated;
        cfg.providers = providers_new;
        return true;
    }
    return false;
...
fn readTokenFromStdin(allocator: std.mem.Allocator) []u8 {
    ...
    while (used < buffer.len) {
        const read_n = stdin.read(buffer[used..]) catch {
            std.debug.print("Failed to read token from stdin.\n", .{});
            std.process.exit(1);
        };
        if (read_n == 0) break;
        used += read_n;
    ...
    const first_line = if (std.mem.indexOfScalar(u8, buffer[0..used], '\n')) |idx|
        buffer[0..idx]
    else
        buffer[0..used];
    const trimmed = std.mem.trim(u8, first_line, " \t\r\n");
    return allocator.dupe(u8, trimmed) catch {
        std.debug.print("Out of memory while storing token.\n", .{});
        std.process.exit(1);
    };
}

fn readTokenInteractive(allocator: std.mem.Allocator, provider_name: []const u8) []u8 {
    std.debug.print("Enter API key/token for {s}: ", .{provider_name});
    std.debug.print("\n", .{});
    std.debug.print("(Input is read from terminal. For non-echo shell piping use --stdin.)\n", .{});
    return readTokenFromStdin(allocator);
}
...
fn runAuthDeviceCodeLogin(
    ...
    const dc = auth_mod.startDeviceCodeFlow(
        ...
        std.debug.print("  cerberus auth login openai-codex --import-codex   (import from Codex CLI)\n", .{});
        std.process.exit(1);
    };
...
fn runAuthImportPi(
    ...
    defer if (loaded_path) |p| allocator.free(p);
    var json_bytes: ?[]u8 = null;
    defer if (json_bytes) |b| allocator.free(b);

    for (candidate_paths) |parts| {
        const path = std.fs.path.join(allocator, parts) catch {
            std.debug.print("Out of memory.\n", .{});
            std.process.exit(1);
        };
        ...
        const bytes = file.readToEndAlloc(allocator, 1024 * 1024) catch {
            allocator.free(path);
            continue;
        };
    ...
    var token_obj = root_obj;
    if (root_obj.get("tokens")) |tokens_val| {
        token_obj = switch (tokens_val) {
            .object => |o| o,
            else => token_obj,
        };
    } else if (root_obj.get("oauth")) |oauth_val| {
        token_obj = switch (oauth_val) {
            .object => |o| o,
            else => token_obj,
        };
    }
    ...
    const refresh_token_str: ?[]const u8 = if (token_obj.get("refresh_token")) |rt_val| switch (rt_val) {
        .string => |s| if (s.len > 0) s else null,
        else => null,
    } else null;
...
fn printUsage() void {
    const usage =
        \\cerberus -- The smallest AI assistant. Zig-powered.
        \\
        \\USAGE:
        \\  cerberus <command> [options]
        \\
        \\COMMANDS:
...
test "applyGatewayDaemonOverrides applies CLI port before validation" {
    var cfg = yc.config.Config{
        .workspace_dir = "/tmp/cerberus-test",
        .config_path = "/tmp/cerberus-test/config.json",
