...
const MEMORY_TEMPLATE =
    ...
    \\- **notes**: Add durable facts with `cerberus memory add <key> <content>`.
    \\
;
...
fn print_usage() !void {
    try print_out(
        ...
        \\  cerberus version
        \\  cerberus help
        \\  cerberus onboard [--workspace PATH]
        \\  cerberus status [--workspace PATH]
        \\  cerberus identity <show|set TEXT...> [--workspace PATH]
        \\  cerberus memory <add|list|search|clear> [...] [--workspace PATH]
        \\  cerberus agent -m "message" [--workspace PATH]
        \\
...
fn run_onboard(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    if (parsed.positionals.len != 0) {
        try print_err("Usage: cerberus onboard [--workspace PATH]\n", .{});
...
fn run_status(allocator: std.mem.Allocator, args: []const []const u8) !void {
    const parsed = try parse_workspace_args(allocator, args);
    if (parsed.positionals.len != 0) {
        try print_err("Usage: cerberus status [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
    ...
    try print_out("cerberus WASI status\n", .{});
    try print_out("workspace: {s}\n", .{parsed.workspace});
...
fn run_identity(allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    if (parsed.positionals.len < 1) {
        try print_err("Usage: cerberus identity <show|set TEXT...> [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
    ...
    if (std.mem.eql(u8, subcmd, "set")) {
        if (parsed.positionals.len < 2) {
            try print_err("Usage: cerberus identity set TEXT... [--workspace PATH]\n", .{});
            return error.InvalidUsage;
        }
...
fn run_memory(allocator: std.mem.Allocator, args: []const []const u8) !void {
    const parsed = try parse_workspace_args(allocator, args);
    if (parsed.positionals.len < 1) {
        try print_err("Usage: cerberus memory <add|list|search|clear> ... [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
    ...
    if (std.mem.eql(u8, subcmd, "add")) {
        if (parsed.positionals.len < 3) {
            try print_err("Usage: cerberus memory add <key> <content...> [--workspace PATH]\n", .{});
            return error.InvalidUsage;
        }
    ...
    if (std.mem.eql(u8, subcmd, "search")) {
        if (parsed.positionals.len < 2) {
            try print_err("Usage: cerberus memory search <query...> [--workspace PATH]\n", .{});
            return error.InvalidUsage;
        }
...
fn run_agent(allocator: std.mem.Allocator, args: []const []const u8) !void {
    const parsed = try parse_workspace_args(allocator, args);
    if (parsed.positionals.len < 2) {
        try print_err("Usage: cerberus agent -m \"message\" [--workspace PATH]\n", .{});
        return error.InvalidUsage;
    }
