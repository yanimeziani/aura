...
fn appendSkillsSection(
    ...
    // Two-source loading: workspace skills + ~/.cerberus/skills/community/
    ...
    const community_base = if (home_dir) |h|
        std.fs.path.join(allocator, &.{ h, ".cerberus", "skills" }) catch null
    else
        null;
...
test "appendSkillsSection with no skills produces nothing" {
    ...
    const w = buf.writer(allocator);
    try appendSkillsSection(allocator, w, "/tmp/cerberus-prompt-test-no-skills");
...
test "appendSkillsSection renders unavailable skill with missing deps" {
    ...
    // Skill requiring nonexistent binary and env
    {
        ...
        defer f.close();
        try f.writeAll("{\"name\": \"docker-deploy\", \"description\": \"Deploy with docker\", \"requires_bins\": [\"cerberus_fake_docker_xyz\"], \"requires_env\": [\"CERBERUS_FAKE_TOKEN_XYZ\"]}");
    }
...
test "appendSkillsSection unavailable always=true skill renders in XML not full" {
    ...
    // always=true but requires nonexistent binary — should be unavailable
    {
        ...
        defer f.close();
        try f.writeAll("{\"name\": \"broken-always\", \"description\": \"Broken always skill\", \"always\": true, \"requires_bins\": [\"cerberus_nonexistent_xyz_aaa\"]}");
    }
    {
