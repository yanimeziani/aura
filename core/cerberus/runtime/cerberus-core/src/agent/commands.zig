...
test "parseSlashCommand strips bot mention from command name" {
    const parsed = parseSlashCommand("/model@cerberus_bot openrouter/inception/mercury") orelse return error.TestExpectedEqual;
...
test "parseSlashCommand strips bot mention with colon separator" {
    const parsed = parseSlashCommand("/model@cerberus_bot: gpt-5.2") orelse return error.TestExpectedEqual;
    try std.testing.expectEqualStrings("model", parsed.name);
...
test "bareSessionResetPrompt returns prompt for bare /reset with mention" {
    const prompt = bareSessionResetPrompt("/reset@cerberus_bot:") orelse return error.TestExpectedEqual;
