...
//
// Each skill lives in ~/.cerberus/workspace/skills/<name>/ with:
...
pub fn syncCommunitySkills(allocator: std.mem.Allocator, workspace_dir: []const u8) !void {
    ...
    const enabled_env = platform.getEnvOrNull(allocator, "CERBERUS_OPEN_SKILLS_ENABLED");
    ...
    const community_dir = blk: {
        if (platform.getEnvOrNull(allocator, "CERBERUS_OPEN_SKILLS_DIR")) |dir| {
            break :blk dir;
        }
...
test "parseManifest full JSON" {
    const json =
        \\{"name": "code-review", "version": "1.2.0", "description": "Automated code review", "author": "cerberus"}
    ;
    ...
    try std.testing.expectEqualStrings("cerberus", m.author);
}
...
test "listSkills from nonexistent directory" {
    ...
    const skills = try listSkills(allocator, "/tmp/cerberus-test-skills-nonexistent-dir");
...
test "readSyncMarker returns null for nonexistent file" {
    var buf: [256]u8 = undefined;
    const ts = readSyncMarker("/tmp/cerberus-nonexistent-marker-file.json", &buf);
    try std.testing.expect(ts == null);
}

test "syncCommunitySkills disabled when env not set" {
    // CERBERUS_OPEN_SKILLS_ENABLED is not set in test environment,
    // so syncCommunitySkills should return immediately without doing anything
    const allocator = std.testing.allocator;
    try syncCommunitySkills(allocator, "/tmp/cerberus-test-sync-disabled");
    // No error = success (function returned early)
}

test "loadCommunitySkills from nonexistent directory" {
    const allocator = std.testing.allocator;
    const skills = try loadCommunitySkills(allocator, "/tmp/cerberus-test-community-nonexistent");
    defer freeSkills(allocator, skills);
    try std.testing.expectEqual(@as(usize, 0), skills.len);
}
...
test "checkRequirements detects missing env var" {
    ...
    env_arr[0] = try allocator.dupe(u8, "CERBERUS_TEST_NONEXISTENT_VAR_XYZ123");
    var skill = Skill{
        .name = "needs-env",
    ...
    try std.testing.expect(!skill.available);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "env:CERBERUS_TEST_NONEXISTENT_VAR_XYZ123") != null);
}
...
test "checkBinaryExists returns false for nonexistent binary" {
    const allocator = std.testing.allocator;
    try std.testing.expect(!checkBinaryExists(allocator, "cerberus_nonexistent_binary_xyz"));
}
...
test "listSkillsMerged with nonexistent dirs returns empty" {
    const allocator = std.testing.allocator;
    const skills = try listSkillsMerged(allocator, "/tmp/cerberus-nonexistent-a", "/tmp/cerberus-nonexistent-b");
    defer freeSkills(allocator, skills);
    try std.testing.expectEqual(@as(usize, 0), skills.len);
}
...
test "checkRequirements detects missing binary" {
    ...
    bin_arr[0] = try allocator.dupe(u8, "cerberus_nonexistent_xyz_bin");
    var skill = Skill{
        .name = "needs-bin",
    ...
    try std.testing.expect(!skill.available);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "bin:cerberus_nonexistent_xyz_bin") != null);
}

test "checkRequirements detects both missing bin and env" {
    const allocator = std.testing.allocator;
    const bin_arr = try allocator.alloc([]const u8, 1);
    bin_arr[0] = try allocator.dupe(u8, "cerberus_missing_bin_abc");
    const env_arr = try allocator.alloc([]const u8, 1);
    env_arr[0] = try allocator.dupe(u8, "CERBERUS_MISSING_ENV_ABC");
    var skill = Skill{
        .name = "needs-both",
        .requires_bins = bin_arr,
        .requires_env = env_arr,
    ...
    try std.testing.expect(!skill.available);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "bin:cerberus_missing_bin_abc") != null);
    try std.testing.expect(std.mem.indexOf(u8, skill.missing_deps, "env:CERBERUS_MISSING_ENV_ABC") != null);
}
...
test "listSkillsMerged runs checkRequirements" {
...

    {
        ...
        const f = try tmp.dir.createFile(rel, .{});
        defer f.close();
        try f.writeAll("{\"name\": \"needy\", \"description\": \"needs stuff\", \"requires_bins\": [\"cerberus_fake_bin_zzz\"]}");
    }
    ...
    try std.testing.expect(std.mem.indexOf(u8, skills[0].missing_deps, "bin:cerberus_fake_bin_zzz") != null);
}
...
test "syncCommunitySkillsResult disabled when env not set" {
    // CERBERUS_OPEN_SKILLS_ENABLED is not set in test environment
    const allocator = std.testing.allocator;
    const result = try syncCommunitySkillsResult(allocator, "/tmp/cerberus-test-sync-result-disabled");
    defer freeSyncResult(allocator, &result);
...
test "countMdFiles returns zero for nonexistent dir" {
    const count = countMdFiles("/tmp/cerberus-test-countmd-nonexistent");
