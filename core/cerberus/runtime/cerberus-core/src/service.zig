...
const SERVICE_LABEL = "com.cerberus.daemon";
const WINDOWS_SERVICE_NAME = "cerberus";
const WINDOWS_SERVICE_DISPLAY_NAME = "cerberus gateway runtime";
...
fn startService(allocator: std.mem.Allocator) !void {
    if (comptime builtin.os.tag == .macos) {
        ...
        try runChecked(allocator, &.{ "systemctl", "--user", "start", "cerberus.service" });
    } else if (comptime builtin.os.tag == .windows) {
...
fn stopService(allocator: std.mem.Allocator) !void {
    if (comptime builtin.os.tag == .macos) {
        ...
        try assertLinuxSystemdUserAvailable(allocator);
        try runChecked(allocator, &.{ "systemctl", "--user", "stop", "cerberus.service" });
    } else if (comptime builtin.os.tag == .windows) {
...
fn stopServiceForRestart(allocator: std.mem.Allocator) !void {
    if (comptime builtin.os.tag == .macos) {
        ...
        try assertLinuxSystemdUserAvailable(allocator);
        const status = try runCaptureStatus(allocator, &.{ "systemctl", "--user", "stop", "cerberus.service" });
...
fn serviceStatus(allocator: std.mem.Allocator) !void {
    ...
    if (comptime builtin.os.tag == .macos) {
        ...
        try assertLinuxSystemdUserAvailable(allocator);
        const output = runCapture(allocator, &.{ "systemctl", "--user", "is-active", "cerberus.service" }) catch try allocator.dupe(u8, "unknown");
...
fn installMacos(allocator: std.mem.Allocator, _: []const u8) !void {
    ...
    const logs_dir = try std.fmt.allocPrint(allocator, "{s}/.cerberus/logs", .{home});
...
fn installLinux(allocator: std.mem.Allocator) !void {
    ...
    const content = try std.fmt.allocPrint(allocator,
        \\[Unit]
        \\Description=cerberus gateway runtime
    ...
    try runChecked(allocator, &.{ "systemctl", "--user", "daemon-reload" });
    try runChecked(allocator, &.{ "systemctl", "--user", "enable", "cerberus.service" });
}
...
fn linuxServiceFile(allocator: std.mem.Allocator) ![]const u8 {
    const home = try getHomeDir(allocator);
    defer allocator.free(home);
    return std.fs.path.join(allocator, &.{ home, ".config", "systemd", "user", "cerberus.service" });
}
...
fn isSystemdUnitNotLoadedDetail(detail: []const u8) bool {
    return std.ascii.indexOfIgnoreCase(detail, "unit cerberus.service not loaded") != null or
...
test "service label is set" {
    ...
    try std.testing.expect(std.mem.indexOf(u8, SERVICE_LABEL, "cerberus") != null);
}
...
test "linuxServiceFile contains service suffix" {
    ...
    defer std.testing.allocator.free(path);
    try std.testing.expect(std.mem.endsWith(u8, path, "cerberus.service"));
}

test "xmlEscape returns input for safe strings" {
    const input = "/usr/local/bin/cerberus";
...
test "isSystemdUnavailableDetail detects common unavailable errors" {
    ...
    try std.testing.expect(!isSystemdUnavailableDetail("unit cerberus.service not found"));
...
test "isSystemdUnitNotLoadedDetail detects stop-not-loaded patterns" {
    try std.testing.expect(isSystemdUnitNotLoadedDetail("Unit cerberus.service not loaded."));
    try std.testing.expect(isSystemdUnitNotLoadedDetail("Unit cerberus.service could not be found."));
