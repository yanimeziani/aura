...
/// Scout: search GitHub for cerberus-compatible skill repositories.
/// Uses api.github.com/search/repositories?q=QUERY+topic:cerberus
pub fn scout(allocator: std.mem.Allocator, query: []const u8) !std.ArrayList(SkillCandidate) {
    ...
    const url = try std.fmt.allocPrint(
        allocator,
        "https://api.github.com/search/repositories?q={s}+topic:cerberus&sort=stars&order=desc&per_page=30",
        .{encoded_query},
    );
    ...
    const result = client.fetch(.{
        ...
        .method = .GET,
        .extra_headers = &.{
            .{ .name = "User-Agent", .value = "cerberus/0.1" },
...
pub fn forge(allocator: std.mem.Allocator, cfg: SkillForgeConfig) !ForgeReport {
    ...
    // Scout phase
    var candidates = try scout(allocator, "cerberus skill");
...
test "buildGitHubSearchUrl encodes query" {
    const allocator = std.testing.allocator;
    const url = try buildGitHubSearchUrl(allocator, "cerberus skill");
    defer allocator.free(url);
    try std.testing.expect(std.mem.indexOf(u8, url, "cerberus+skill") != null);
    try std.testing.expect(std.mem.indexOf(u8, url, "api.github.com") != null);
    try std.testing.expect(std.mem.indexOf(u8, url, "topic:cerberus") != null);
}
