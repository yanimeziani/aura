...
test "prepareMessagesForProvider does not delete cerberus temp image files" {
    var tmp_dir = std.testing.tmpDir(.{});
    defer tmp_dir.cleanup();

    try tmp_dir.dir.writeFile(.{
        .sub_path = "cerberus_photo_123.png",
        .data = "\x89PNG\x0d\x0a\x1a\x0a",
    });

    const dir_path = try tmp_dir.dir.realpathAlloc(std.testing.allocator, ".");
    defer std.testing.allocator.free(dir_path);
    const file_path = try std.fs.path.join(std.testing.allocator, &.{ dir_path, "cerberus_photo_123.png" });
    defer std.testing.allocator.free(file_path);
