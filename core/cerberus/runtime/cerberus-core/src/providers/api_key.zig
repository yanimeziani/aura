...
/// 3. Generic fallback variables (`CERBERUS_API_KEY`, `API_KEY`)
pub fn resolveApiKey(
    ...
    // 3. Generic fallbacks
    const fallbacks = [_][]const u8{ "CERBERUS_API_KEY", "API_KEY" };
...
///   3. Generic fallbacks (CERBERUS_API_KEY, API_KEY)
    ...
    for (providers) |e| {
        if (std.mem.eql(u8, e.name, provider_name)) {
            if (e.api_key) |k| {
                ...
                if (secrets.SecretStore.isEncrypted(trimmed)) {
                    ...
                    return try store.decryptSecret(allocator, trimmed);
                }
...
test "resolveApiKeyFromConfig trims configured key" {
    const entries = [_]config_mod.ProviderEntry{
        .{ .name = "openai", .api_key = "   sk-trimmed   " },
    };
    const result = try resolveApiKeyFromConfig(std.testing.allocator, "openai", &entries);
    defer if (result) |r| std.testing.allocator.free(r);
    try std.testing.expectEqualStrings("sk-trimmed", result.?);
}
