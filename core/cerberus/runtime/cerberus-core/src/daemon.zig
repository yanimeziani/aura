...
/// Run the long-lived runtime. This is the main entry point for `cerberus gateway`.
...
pub fn run(allocator: std.mem.Allocator, config: *const Config, host: []const u8, port: u16) !void {
    ...
    try stdout.print("cerberus gateway runtime started\n", .{});
    ...
    try stdout.print("cerberus gateway runtime stopped.\n", .{});
}
...
test "stateFilePath derives from config_path" {
    const config = Config{
        ...
        .config_path = "/home/user/.cerberus/config.json",
        .allocator = std.testing.allocator,
    };
    const path = try stateFilePath(std.testing.allocator, &config);
    defer std.testing.allocator.free(path);
    const expected = try std.fs.path.join(std.testing.allocator, &.{ "/home/user/.cerberus", "daemon_state.json" });
