...
/// Get the default cron.json path: ~/.cerberus/cron.json
fn cronJsonPath(allocator: std.mem.Allocator) ![]const u8 {
    const home = try platform.getHomeDir(allocator);
    defer allocator.free(home);
    return std.fs.path.join(allocator, &.{ home, ".cerberus", "cron.json" });
}

/// Ensure the ~/.cerberus directory exists.
fn ensureCronDir(allocator: std.mem.Allocator) !void {
    const home = try platform.getHomeDir(allocator);
    defer allocator.free(home);
    const dir = try std.fs.path.join(allocator, &.{ home, ".cerberus" });
...
/// Save scheduler jobs to ~/.cerberus/cron.json.
...
/// Load jobs from ~/.cerberus/cron.json into the scheduler.
pub fn loadJobs(scheduler: *CronScheduler) !void {
    try loadJobsWithPolicy(scheduler, .best_effort);
}

/// Load jobs from ~/.cerberus/cron.json; unlike loadJobs, this returns
...
pub fn cliListJobs(allocator: std.mem.Allocator) !void {
    ...
    if (jobs.len == 0) {
        ...
        log.info("Usage:", .{});
        log.info("  cerberus cron add '*/10 * * * *' 'echo hello'", .{});
        log.info("  cerberus cron once 30m 'echo reminder'", .{});
        return;
    }
