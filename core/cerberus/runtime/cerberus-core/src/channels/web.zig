...
pub const WebChannel = struct {
    ...
    fn loadLocalTokenFromEnv(self: *WebChannel) !bool {
        return self.loadTokenFromEnvCandidates(&.{
            "CERBERUS_WEB_TOKEN",
            "CERBERUS_GATEWAY_TOKEN",
            "OPENCLAW_GATEWAY_TOKEN",
        });
    }

    fn loadDedicatedRelayTokenFromEnv(self: *WebChannel) !?[]u8 {
        if (std.process.getEnvVarOwned(self.allocator, "CERBERUS_RELAY_TOKEN")) |raw| {
            if (!config_types.WebConfig.isValidAuthToken(raw)) {
                log.warn("Ignoring invalid relay token from env CERBERUS_RELAY_TOKEN", .{});
    ...
    fn handleRelayPairingRequest(self: *WebChannel, session_id: []const u8, request_id: ?[]const u8, payload_obj: ?std.json.ObjectMap) void {
        ...
        const cookie = std.fmt.allocPrint(
            self.allocator,
            "cerberus_ui_token={s}; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age={d}",
    ...
    fn ensureLocalTokenSourceCompatible(self: *const WebChannel, token_source: LocalTokenSource) !void {
        if (self.message_auth_mode == .token and token_source == .ephemeral) {
            log.warn("Web channel message_auth_mode=token requires stable auth token from config (channels.web.auth_token) or env (CERBERUS_WEB_TOKEN/CERBERUS_GATEWAY_TOKEN/OPENCLAW_GATEWAY_TOKEN)", .{});
            return error.InvalidConfiguration;
        }
    }
    ...
    fn startRelayTransport(self: *WebChannel) !void {
        ...
        switch (resolved.source) {
            ...
            .env => log.info("Web relay token loaded from env CERBERUS_RELAY_TOKEN and persisted", .{}),
        ...
        const agent_header = try std.fmt.bufPrint(&agent_header_buf, "X-Cerberus-Agent: {s}", .{self.relay_agent_id});
...
test "WebChannel initFromConfig maps relay transport settings" {
    ...
    try std.testing.expectEqualStrings("wss://relay.cerberus.io/ws/agent", ch.relay_url.?);
...
test "parseRelayEndpoint parses host path and default port" {
    const ep = try parseRelayEndpoint("wss://relay.cerberus.io/ws/agent");
    try std.testing.expectEqualStrings("relay.cerberus.io", ep.host);
...
test "parseRelayEndpoint parses explicit port" {
    const ep = try parseRelayEndpoint("wss://relay.cerberus.io:9443/ws");
    try std.testing.expectEqualStrings("relay.cerberus.io", ep.host);
...
test "parseRelayEndpoint rejects non-wss URL" {
    try std.testing.expectError(error.InvalidRelayUrl, parseRelayEndpoint("https://relay.cerberus.io/ws"));
    try std.testing.expectError(error.InvalidRelayUrl, parseRelayEndpoint("ws://relay.cerberus.io/ws"));
}
