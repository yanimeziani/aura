...
fn sweepTempMediaFilesInDir(dir_path: []const u8, now_secs: i64, ttl_secs: i64) void {
    ...
    while (iter.next() catch null) |entry| {
        ...
        if (!std.mem.startsWith(u8, entry.name, "cerberus_doc_") and
            !std.mem.startsWith(u8, entry.name, "cerberus_photo_"))
            continue;
...
fn downloadTelegramPhoto(allocator: std.mem.Allocator, bot_token: []const u8, file_id: []const u8, proxy: ?[]const u8) ?[]u8 {
    ...
    path_fbs.writer().print("{s}{s}cerberus_photo_{s}{s}", .{ tmp_base, pathSeparator(tmp_base), safe_name, ext }) catch return null;
...
fn downloadTelegramFile(allocator: std.mem.Allocator, bot_token: []const u8, file_id: []const u8, file_name: ?[]const u8, proxy: ?[]const u8) ?[]u8 {
    ...
    if (file_name) |fname| {
        ...
        const tmp_base = trimTrailingPathSeparators(tmp_dir);
        path_fbs.writer().print("{s}{s}cerberus_doc_{s}_{s}", .{ tmp_base, pathSeparator(tmp_base), safe_id_part, safe_name }) catch return null;
    } else {
        ...
        const safe_name = sanitizeFilenameComponent(&name_buf, file_id, 200);
        const tmp_base = trimTrailingPathSeparators(tmp_dir);
        path_fbs.writer().print("{s}{s}cerberus_doc_{s}{s}", .{ tmp_base, pathSeparator(tmp_base), safe_name, ext }) catch return null;
    }
    const local_path = path_fbs.getWritten();
...
test "telegram parseAttachmentMarkers FILE with caption" {
    const parsed = try parseAttachmentMarkers(
        std.testing.allocator,
        "[FILE:/tmp/cerberus_doc_report.docx] Вот документ",
    );
    defer parsed.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(usize, 1), parsed.attachments.len);
    try std.testing.expectEqual(AttachmentKind.document, parsed.attachments[0].kind);
    try std.testing.expectEqualStrings("/tmp/cerberus_doc_report.docx", parsed.attachments[0].target);
}
...
test "telegram parseAttachmentMarkers FILE with cyrillic filename" {
    const parsed = try parseAttachmentMarkers(
        std.testing.allocator,
        "[FILE:/tmp/cerberus_doc_Справка_в_школы.docx]",
    );
    defer parsed.deinit(std.testing.allocator);

    try std.testing.expectEqual(@as(usize, 1), parsed.attachments.len);
    try std.testing.expectEqualStrings("/tmp/cerberus_doc_Справка_в_школы.docx", parsed.attachments[0].target);
}
...
test "telegram sweepTempMediaFilesInDir removes only stale cerberus temp media files" {
    var tmp_dir = std.testing.tmpDir(.{});
    defer tmp_dir.cleanup();

    try tmp_dir.dir.writeFile(.{ .sub_path = "cerberus_doc_old.txt", .data = "doc" });
    try tmp_dir.dir.writeFile(.{ .sub_path = "cerberus_photo_old.jpg", .data = "photo" });
    ...
    const doc_stat = tmp_dir.dir.statFile("cerberus_doc_old.txt");
    try std.testing.expectError(error.FileNotFound, doc_stat);

    const photo_stat = tmp_dir.dir.statFile("cerberus_photo_old.jpg");
