...
Deploy cerberus with multiple agents in a shared Matrix room on [Modal](https://modal.com).
...
Secrets flow: `.env` (local) → `modal.Secret.from_dotenv()` → container env vars → `inject_secrets()` patches config at startup → cerberus starts with real credentials. Patched config is written only inside the running container (`/cerberus-data/.cerberus/config.json`) and is not tracked in git.
...
```sh
ssh root@cerberus-modal   # or whatever TAILSCALE_HOSTNAME you set
```

This gives you a shell in the running container — inspect logs, debug cerberus, check the workspace, etc.
...
| `cerberus-linux-musl` | no | Cross-compiled binary |
