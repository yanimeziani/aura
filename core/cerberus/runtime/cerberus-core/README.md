<p align="center">
  <img src="cerberus.png" alt="cerberus" width="200" />
</p>
...
<p align="center">
  <a href="https://github.com/cerberus/cerberus/actions/workflows/ci.yml"><img src="https://github.com/cerberus/cerberus/actions/workflows/ci.yml/badge.svg" alt="CI" /></a>
  <a href="https://cerberus.github.io"><img src="https://img.shields.io/badge/docs-cerberus.github.io-informational" alt="Documentation" /></a>
...
| | OpenClaw (legacy) | [NanoBot](https://github.com/HKUDS/nanobot) | [PicoClaw](https://github.com/sipeed/picoclaw) | [ZeroClaw](https://github.com/zeroclaw-labs/zeroclaw) | **[🦞 Cerberus](https://github.com/cerberus/cerberus)** |
|---|---|---|---|---|---|
...
> Measured with `/usr/bin/time -l` on ReleaseSmall builds. cerberus is a static binary with zero runtime dependencies.

Reproduce locally:

```bash
zig build -Doptimize=ReleaseSmall
ls -lh zig-out/bin/cerberus

/usr/bin/time -l zig-out/bin/cerberus --help
/usr/bin/time -l zig-out/bin/cerberus status
```
...
```bash
git clone https://github.com/cerberus/cerberus.git
cd cerberus
zig build -Doptimize=ReleaseSmall
...
# Chat
cerberus agent -m "Hello, cerberus!"

# Interactive mode
cerberus agent

# Start gateway runtime (gateway + all configured channels/accounts + heartbeat + scheduler)
cerberus gateway                # default: 127.0.0.1:3000
cerberus gateway --port 8080    # custom port
...
cerberus channel status

# Start specific channels
cerberus channel start telegram
cerberus channel start discord
cerberus channel start signal

# Manage background service
cerberus service install
cerberus service status

# Migrate memory from OpenClaw
cerberus migrate openclaw --dry-run
cerberus migrate openclaw
```

> **Dev fallback (no global install):** prefix commands with `zig-out/bin/` (example: `zig-out/bin/cerberus status`).
...
| **Identity** | `IdentityConfig` | Cerberus (markdown), AIEOS v1.1 (JSON) | Any identity format |
...
cerberus enforces security at **every layer**.
...
`cerberus` speaks Nostr natively via NIP-17 (gift-wrapped private DMs) and NIP-04 (legacy DMs), using [`nak`](https://github.com/fiatjaf/nak).
...
```bash
cerberus onboard --interactive   # Step 7 configures Nostr
```
...
**How it works:** On startup, cerberus announces its DM inbox relays (kind:10050), then listens for incoming NIP-17 gift wraps and NIP-04 encrypted DMs. Outbound messages mirror the sender's protocol. Multi-relay rumor deduplication prevents duplicate responses when the same message is delivered via multiple relays.

## Configuration

Config: `~/.cerberus/config.json` (created by `onboard`)

> **Config format:** cerberus uses a snake_case JSON config structure. Providers live under `models.providers`, the default model under `agents.defaults.model.primary`, and channels use `accounts` wrappers.
...
- For orchestration, use local token mode with a stable token from config or env (`CERBERUS_WEB_TOKEN`, `CERBERUS_GATEWAY_TOKEN`).
  ...
  "channels": {
    "web": {
      "accounts": {
        "default": {
          "transport": "relay",
          "relay_url": "wss://relay.cerberus.io/ws/agent",
...
- Relay token lifecycle (dedicated): `relay_token` (config) -> `CERBERUS_RELAY_TOKEN` (env) -> persisted `web-relay-<account_id>` credential -> generated token.
...
| `migrate openclaw [--dry-run] [--source PATH]` | Import memory + migrate config from legacy OpenClaw |
...
- `src/config.zig` (`channels.*.accounts` parsing, multi-account selection/ordering, aliases)
...
cerberus uses **CalVer** (`YYYY.M.D`) for releases — e.g. `v2026.2.20`.
...
- **`cerberus --version`** prints the current version
...
- New `Skill` -> `~/.cerberus/workspace/skills/<name>/`

## Disclaimer

cerberus is a pure open-source software project. It has **no token, no cryptocurrency, no blockchain component, and no financial instrument** of any kind. This project is not affiliated with any token or financial product.
...
**cerberus** — Null overhead. Null compromise. Deploy anywhere. Swap anything.

## Star History

[![Star History Chart](https://api.star-history.com/svg?repos=cerberus/cerberus&type=date&legend=top-left)](https://www.star-history.com/#cerberus/cerberus&type=date&legend=top-left)
