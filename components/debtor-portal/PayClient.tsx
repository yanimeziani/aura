...
export default function PayClient({ debtorId, token, contractSnippet }: Props) {
  ...
  const chatHref = `/chat/${debtorId}?token=${encodeURIComponent(token)}`;
  ...
  return (
    <main className="min-h-screen bg-base-100 text-base-content">
      <div className="app-shell max-w-6xl py-8 sm:py-16">
        <Link href={chatHref} className="btn btn-ghost gap-2 mb-10 min-h-11">
        ...
        {payError && (
          <div className="alert alert-error mb-8 max-w-2xl mx-auto shadow-warm">
            ...
            <button className="btn btn-ghost min-h-10" onClick={() => setPayError(null)}>
              {t('dismiss')}
            </button>
          </div>
        )}
          ...
          {plans.map((plan) => (
            <div
              ...
              className={`card bg-base-200/50 border shadow-warm transition-shadow hover:shadow-elevated ${
                plan.featured ? 'border-primary/30 ring-1 ring-primary/10 scale-[1.02]' : 'border-base-300/50'
              }`}
            >
              {plan.featured && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="badge badge-primary badge-sm font-bold">{t('recommended')}</span>
                </div>
              )}
              <div className="card-body p-6 sm:p-8 gap-5">
                ...
                <div>
                  <span className="text-3xl sm:text-4xl font-bold tracking-tight">{plan.display}</span>
                  {plan.suffix && <span className="text-sm text-base-content/40 ml-1">{plan.suffix}</span>}
                  {plan.strikethrough && (
                    <p className="text-sm text-base-content/30 line-through mt-1">{plan.strikethrough}</p>
                  )}
                </div>
                ...
                <button
                  ...
                  disabled={paying}
                  className={`btn w-full gap-2 min-h-12 ${plan.featured ? 'btn-primary' : 'btn-ghost border border-base-300/50'}`}
                >
                  {paying ? (
        ...
        <div className="mt-16 text-center max-w-2xl mx-auto">
          <div className="card bg-base-200/30 border border-base-300/30">
            <div className="card-body p-6 items-center text-center">
              ...
              <p className="text-[10px] text-base-content/35 mt-3 max-w-sm">
                {t('verifyHint', { merchant: debtor.merchant.name })}
              </p>
            </div>
          </div>
        </div>
