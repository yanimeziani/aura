...
OpenClaw automatically shifts to minimum-cost operations until the next day's
budget resets.
...
| Manual: `openclaw panic on` | Activate panic mode |
...
Manual deactivation:
```bash
openclaw panic off   # requires Yani confirmation
```
